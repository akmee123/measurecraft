// ================================================================
        ...
        function isCountElement(el) {
            return !!(el && typeof el.type === 'string' && el.type.indexOf('count_') === 0);
        }

        function selectAllElements() {
            // Include manual count markers even when hidden (backing box not drawn).
            selectedIds = elements.filter(function (el) {
                if (!el) return false;
                if (isCountElement(el)) return true;
                return !el.hidden;
        ...
        function deleteSelected() {
            ...
            saveState();
            const toDelete = new Set(selectedIds.map(function (id) { return String(id); }));
            selectedIds.forEach(id => {
                getAttachedChildIds(id).forEach(cid => toDelete.add(String(cid)));
            });
            elements.forEach(el => {
                if (el.parentId != null && toDelete.has(String(el.parentId))) toDelete.add(String(el.id));
            });
            elements.forEach(function (el) {
                if (el && isCountElement(el) && toDelete.has(String(el.id))) el.locked = false;
            });
            elements.forEach(el => {
                if (el.cutouts) el.cutouts = el.cutouts.filter(cid => !toDelete.has(String(cid)));
            });
            const removed = elements.filter(el => toDelete.has(String(el.id)));
            elements = elements.filter(el => !toDelete.has(String(el.id)));
            selectedIds = [];
            try {
                if (window.MCCountTool && typeof window.MCCountTool.onElementsDeleted === 'function') {
                    window.MCCountTool.onElementsDeleted(removed.map(function (el) { return el.id; }));
                }
            } catch (_) {}
            renderAll();
            try {
        ...
        function elementHitsPoint(el, world) {
            if (!el) return false;
            if (el.hidden && !(typeof el.type === 'string' && el.type.indexOf('count_') === 0)) return false;
            // Counts are point markers — use a larger pick radius so they are easy to grab/move
            const isCnt = (typeof el.type === 'string' && el.type.indexOf('count_') === 0);
            const tol = (isCnt ? 14 : 6) / Math.max(viewport.scale || 1, 0.001);
            if (el.isLine && el.p1 && el.p2) {
            ...
            if (el.w != null && el.h != null) {
                if (isCnt) {
                    const cx = el.x + (el.w || 1) / 2, cy = el.y + (el.h || 1) / 2;
                    return Math.hypot(world.x - cx, world.y - cy) <= tol;
                }
        ...
        function getElementMetrics(el) {
            const cf = calibrationFactor || 1;
            if (el && typeof el.type === 'string' && el.type.indexOf('count_') === 0) {
                return { gross: 1, cut: 0, net: 1, lengthM: 0, unit: 'Nr', cutoutCount: 0, isCount: true };
            }
        ...
        function renderProperties() {
            ...
            const isCountEl = !!(m && m.isCount) || (typeof el.type === 'string' && el.type.indexOf('count_') === 0);
            const typeLabel = isCountEl
                ? ('Count · ' + String(el.type || '').replace(/^count_/, '').replace(/_/g, ' '))
                : (el.type.charAt(0).toUpperCase() + el.type.slice(1));

            let html = '';
            html += `<div class="prop-group"><label>Object Type</label><input value="${escapeHtml(typeLabel)}" disabled /></div>`;
            ...
            if (isCountEl) {
                html += `<div class="prop-group"><label>Quantities</label>
                    <div class="cost-display">
                        <div style="display:flex;justify-content:space-between;margin-bottom:4px;"><span>Count</span><span class="total">1 Nr</span></div>
                        <div style="font-size:11px;color:var(--text-secondary);margin-top:4px;line-height:1.4;">
                            Manual count marker. Totals by type appear in Live Quantities as <strong>Nr</strong>.
                        </div>
                    </div></div>`;
            } else {
            html += `<div class="prop-group"><label>Quantities</label>
            ...
            if (!isCountEl && el.type === 'wall' && !isConfirmed) {
            ...
            if (!isCountEl && ['wall', 'beam', 'slab', 'column'].includes(el.type) && m.cutoutCount > 0) {
            ...
            if (!isCountEl && el.isLine && el.p1 && el.p2) {
            ...
            if (!isCountEl && (el.type === 'door' || el.type === 'window')) {
            ...
            } else if (!isCountEl && !['door', 'window'].includes(el.type)) {
                let zLabel = 'Height (m)';
        ...
        function renderQuantityTable() {
            ...
            const hasTakeoff = (elements || []).some(e => e && (e.type === 'wall' || e.type === 'slab' || e.type === 'column' || e.type === 'beam' || (typeof e.type === 'string' && e.type.indexOf('count_') === 0)));
            const hasCountRows = Array.isArray(rows) && rows.some(function (r) { return r && (r.unit === 'Nr' || (r.remarks && String(r.remarks).indexOf('Counted manually') >= 0)); });
            if (!hasTakeoff && !hasCountRows) {
                tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-tertiary);padding:14px;">No measured elements yet. Draw walls / slabs / columns, use Count, or run the AI Agent, then open this panel (↑ Quantities).</td></tr>';
                return;
            }
            let html = '';
        ...
        function setupCanvasInteraction() {
            ...
            canvas.addEventListener('mousemove', (e) => {
                ...
                if (dragMode === 'select' && mouseDown && selectedIds.length > 0) {
                    ...
                    if (moved) {
                        // Keep count marker dots glued to the moved backing elements
                        try {
                            if (window.MCCountTool && typeof window.MCCountTool.syncMarkersFromElements === 'function') {
        ...
        function setupContextMenu() {
            ...
            canvas.addEventListener('contextmenu', (e) => {
                ...
                for (let i = elements.length - 1; i >= 0; i--) {
                    const el = elements[i];
                    if (el.hidden && !(typeof el.type === 'string' && el.type.indexOf('count_') === 0)) continue;
        ...
        function renderTree() {
            ...
            filtered.forEach(el => {
                ...
                if (typeof el.type === 'string' && el.type.indexOf('count_') === 0) {
                    key = 'COUNT_' + el.type.slice(6).toUpperCase() + 'S';
                    if (key.endsWith('SS')) key = key.slice(0, -1);
                }
            ...
            for (const [cat, items] of Object.entries(cats)) {
                ...
                items.forEach(function (el) {
                    ...
                    // Count markers use hidden so the 1×1 box is not drawn — do not fade them in the tree
                    const isCnt = (typeof el.type === 'string' && el.type.indexOf('count_') === 0);
                    const hidden = (el.hidden && !isCnt) ? ' style="opacity:0.4;"' : '';
        ...
        function init() {
            if (window.__mcInited) {
                try { hideLoadingOverlay(); } catch (_) {}
                return;
            }
            window.__mcInited = true;
            try {
            ...
            saveState();
            updateCalibDisplay();
            try { renderAll(); } catch (err) { console.error('Initial renderAll failed', err); }
            hideLoadingOverlay();
            try { setupCanvasInteraction(); } catch (err) { console.error('setupCanvasInteraction failed', err); }
            try { setupContextMenu(); } catch (err) { console.error('setupContextMenu failed', err); }
            try { setupToolbar(); } catch (err) { console.error('setupToolbar failed', err); }
        ...
        // Expose for count-tool.js wrappers
        try {
            window.computeQuantities = computeQuantities;
            window.renderQuantityTable = renderQuantityTable;
        ...
        // large script finishes evaluating — always run init in that case.
        (function bootPro() {
            function run() {
                try { init(); } catch (err) {
                    console.error('MeasureCraft init failed', err);
                    try { hideLoadingOverlay(); } catch (_) {}
                }
            }
            ...
            // Absolute fallback so the UI never stays on "Loading..."
            setTimeout(function () {
                try { hideLoadingOverlay(); } catch (_) {}
            }, 800);
        })();
